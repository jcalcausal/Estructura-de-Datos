...
máximo x y = if x<y then y else x
...
factorial x | x < 0 = error "factorial no definido para enteros negativos" 
            | x <=1 = 1
...
{-
   ...
   expresiones:
     ...
     (2 `div` 3, True, 'z')  :: (Double, Bool, Char)
...
inverso x | x == 0 = error "división por cero"
          | otherwise = 1/x
