...
simplify (Node lt1 rt1) (Node lt2 rt2) = Node (Node lt1 rt1) (Node lt2 rt2)
simplify (Node lt1 rt1) (Unif x)       = Node (Node lt1 rt1) (Unif x)
simplify (Unif x) (Node lt1 rt1)       = Node (Unif x) (Node lt1 rt1)
...
get :: Int -> Vector a -> a
get index (V size (Unif x))
  | index < 0 || index >= size = error "error: IndexOutOfBounds: the index is not valid"
  | otherwise                  = x
get index (V size (Node lt rt))
  | index < 0 || index >= size = error "error: IndexOutOfBounds: the index is not valid"
  | index < mid                = get newIndex1 (V mid lt)
  | index >= mid               = get newIndex2 (V mid rt)
  where 
    mid = size `div` 2
    newIndex1 = index
    newIndex2 = index - mid 
...
set index elem (V size tree)
  | index < 0 || index >= size = error "error: IndexOutOfBounds: the index is not valid"
  | otherwise                  = (V size (setTree index elem size tree))

setTree :: (Eq a) => Int -> a -> Int ->  Tree a -> Tree a 
setTree index elem size (Unif x)
  | size == 1   = (Unif elem)
  | index < mid = simplify (setTree newIndex1 elem mid (Unif x)) (Unif x) 
  | otherwise   = simplify (Unif x) (setTree newIndex2 elem mid (Unif x))
  where
    mid       = size `div` 2
    newIndex1 = index
    newIndex2 = index - mid 
setTree index elem size (Node lt rt)
  | index < mid = simplify (setTree newIndex1 elem mid lt) rt
  | otherwise   = simplify lt (setTree newIndex2 elem mid rt)
  where
    mid       = size `div` 2
    newIndex1 = index
    newIndex2 = index - mid   
