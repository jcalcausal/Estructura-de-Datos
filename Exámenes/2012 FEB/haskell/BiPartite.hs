...
aux g dict stack
   ...
   | colored v && notEqual     = Nothing
   | colored v && not notEqual = aux g dict (S.pop stack)
   | otherwise                 = aux g (D.insert v c dict) (pushAll (S.pop stack) suc)
  where
     colored  v         = D.isDefinedAt v dict
     (v, c)             = S.top stack
     notEqual           = not (D.valueOf' v dict == c)
     suc                = [(vertex, color) | vertex <- successors g v, not (colored vertex), let color = nextColor c]
