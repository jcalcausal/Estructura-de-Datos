...
isElem x (Node y restoSet) = x==y || isElem x restoSet
...
insert x (Node y restoSet) 
     |x == y = (Node y restoSet)
     |x < y  = Node x (Node y restoSet)
     |x > y  = Node y (insert x restoSet)
...
delete x (Node y restoSet)
     | x == y = restoSet
     | x < y  = (Node y restoSet)
     | x > y  = Node y (delete x restoSet)
