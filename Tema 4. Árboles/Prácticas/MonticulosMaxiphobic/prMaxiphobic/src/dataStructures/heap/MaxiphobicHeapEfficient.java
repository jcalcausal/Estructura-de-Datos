...
public class MaxiphobicHeapEfficient<T extends Comparable<? super T>> implements Heap<T> {
    ...
    */
    ...
    private static <T extends Comparable<? super T>> Tree<T> merge(Tree<T> h1, Tree<T> h2) {
        if (size(h1) == 0 && size(h2) == 0){  // Caso base con ambos nulos
            return null;
        } else if (size(h1) == 0){           // Caso base con izquierdo nulo
            return h2;
        } else if (size(h2) == 0){           // Caso base con derecho nulo
            return h1;
        } else {                            // Ninguno nulo
            T el;
            Tree<T> l;
            ...
            if (h1.elem.compareTo(h2.elem)<=0){ //el primero pierde
                ...
                if (size(h1.left)>=size(h1.right) && size(h1.left)>=size(h2)){
                    l = h1.left;
                    r = merge(h1.right, h2);
                } else if (size(h1.right)>size(h1.left) && size(h1.right)>=size(h2)) {
                    l = h1.right;
                    r = merge(h1.left, h2);
                } else {
                    l = h2;
                ...
                if (size(h2.left)>=size(h2.right) && size(h2.left)>= size(h1)){
                    l = h2.left;
                    r = merge(h2.right, h1);
                } else if (size(h2.right)>size(h2.left) && size(h2.right)>=size(h1)) {
                    l = h2.right;
                    r = merge(h2.left, h1);
                } else {
                    l = h1;
